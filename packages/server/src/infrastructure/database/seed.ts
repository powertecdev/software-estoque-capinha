...
async function main() {
  ...
  console.log('  ' + phones.length + ' modelos');

  const slots = [];
  for (let i = 1; i <= 20; i++) {
    slots.push(await prisma.slot.create({ data: { label: 'Gancho ' + i } }));
  }
  console.log('  ' + slots.length + ' ganchos');
  ...
  for (const prod of products.slice(0, 8)) {
    await prisma.stockMovement.create({
      data: { productId: prod.id, type: 'ENTRY', quantity: 20, reason: 'Entrada inicial' },
    });
    await prisma.stockMovement.create({
      data: { productId: prod.id, type: 'EXIT', quantity: Math.floor(Math.random() * 5) + 1, reason: 'Venda balcao' },
    });
    movCount += 2;
  }
  console.log('  ' + movCount + ' movimentacoes');
